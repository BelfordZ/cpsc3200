//Problem 4192 Close Enough
//Solution By Zach Belford
//ad-hoc
//for each value, we keep an upper bound and lower bound, based on
//rounding up or down, respectfully. We then test their acceptability
// given an error of up to 0.00000001
#include <iostream>
#include <algorithm>
using namespace std;
...
int main() {
   ...
   while(cin>>cals[0]>>fats[0]>>carbs[0]>>prots[0]) {
	 ...
	 if ( compare(sum[0], cals[1]) < 0 && compare(cals[2], sum[1]) < 1)
	    cout << "yes" << endl;
	 else if (compare(sum[0], cals[0]) < 1 && compare(cals[0], sum[1]) < 0)
	    cout << "yes" << endl;
	 if ( compare(sum[0], cals[1]) < 0 && compare(cals[2], sum[1]) < 1)
	    cout << "yes" << endl;
	 else
